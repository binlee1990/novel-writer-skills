...
- **`--volume vol-XX`**: 仅加载指定卷的时间线事件（Layer 3/2 生效，Layer 1 需手动过滤）
- **`--recent N`**: 显示最近 N 个事件（Layer 3 直接限制，Layer 1/2 取最后 N 条）
...
- **`check`** - 验证时间连续性
  - 检测时间冲突（顺序、并行、持续、跳跃、年龄）
  - `--volume vol-XX`: 仅检查指定卷
  - `--recent N`: 仅检查最近 N 个事件

- **`show`** - 显示时间线概览
  - 生成单线或多线程时间轴可视化
  - `--volume vol-XX`: 仅显示指定卷事件
  - `--recent N`: 仅显示最近 N 个事件
