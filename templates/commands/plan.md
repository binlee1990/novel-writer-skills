...
2. **卷级详细规划**：检查 `$ARGUMENTS` 是否包含 `--detail vol-XX` 或 `--detail vol-XX-YY`
   - 如果包含单卷（`--detail vol-XX`）→ 跳转到「2.2.1 卷级详细规划」章节
   - 如果包含多卷范围（`--detail vol-XX-YY`）→ 跳转到「2.2.1.1 多卷批量规划」章节
   - 如果不包含 → 执行正常的全书规划流程
...
#### 2.2.1.1 多卷批量规划（`--detail vol-XX-YY`）
...
对范围内的每一卷，执行 2.2.1 的 Step 2-4 流程（逐章规划 + 节奏总览 + 写入）。
...
```
💡 下一步建议：
1️⃣ `/tasks` — 将创作计划分解为可执行的写作任务
2️⃣ `/plan --detail vol-01` — 细化第一卷的逐章规划
3️⃣ `/analyze --type=framework` — 验证框架一致性，确认准备就绪
```
