...
>
> 该文件包含 7 层去 AI 规范：
> 1. 禁用词汇表（情绪类、动作类、程度类、转折类、描写类）
> 2. 禁止句式（"然而"开头、"殊不知"上帝视角、"在...下"状语等）
> 3. 形容词限制（每100字最多3个）
> 4. 成语全面禁用
> 5. 对话去AI化
> 6. 段落结构限制
...
**快速记忆**（最常见的 AI 高频词，完整列表见上述文件）：
- 禁用：然而、殊不知、缓缓、深邃、仿佛...一般
- 原则：短句优先、白话替代、删除装饰性形容词
...
在标准命令链式提示之前，检查是否有 P0/P1 级别的推荐：
- 如有 P0 推荐 → 在提示中优先展示（🔴 标记）
- 如有 P1 推荐 → 在提示中附加展示（⚠️ 标记）
- 如无 P0/P1 → 仅展示标准命令链式提示

**数据检查**：
1. 读取 `spec/tracking/character-state.json` → 检查角色缺席章数
2. 读取 `spec/tracking/plot-tracker.json` → 检查伏笔紧急度
3. 读取 `spec/tracking/write-checkpoint.json` → 检查断点状态
...
⚠️ 注意：伏笔「身世之谜」紧急度 0.85，建议近 3 章内回收

💡 下一步建议：
1️⃣ /write [下一章] — 继续写作（建议安排师姐出场）
2️⃣ /recap --brief — 快速查看上下文
