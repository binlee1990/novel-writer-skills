...
专门针对力量体系（修仙境界、魔法等级、超能力等级等）的一致性监控。当 `/analyze --focus=power` 触发时，执行以下深度检查。
...
```
✓ 检查：角色等级描述是否与 specification.md 中的等级体系一致
✓ 检查：角色等级变化是否有合理的升级过程（非跳级）
✓ 检查：同一角色在不同章节中的等级描述是否一致
✓ 检查：等级称谓是否统一（不混用不同叫法）
```
...
✓ 检查：突破条件是否每次都被满足
✓ 检查：力量代价/副作用是否一致体现
✓ 检查：体系限制是否被尊重（不为剧情便利而破坏）
```

### 与 /analyze --focus=power 的协作

当 `/analyze --focus=power` 被触发时：
1. consistency-checker 提供基础的一致性检查框架
2. power-system.md 知识库提供力量体系设计标准
3. 两者结合生成完整的力量体系分析报告
...
问题：等级描述不一致
位置：第 12 章，第 5 段
参考：specification.md 力量体系设定 / 第 8 章等级确立

当前文本："他的金丹期修为全力爆发..."
已建立等级："筑基后期"（第 8 章确立，未描述突破金丹）

可能的解决方案：
1. 在第 9-11 章补充突破金丹的过程
2. 将第 12 章的等级描述改为"筑基后期"
3. 如果是有意的跳跃，在 specification.md 中更新等级时间线
```
