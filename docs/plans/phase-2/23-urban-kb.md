# Task 23: 新增 `urban.md` 知识库

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** 创建都市小说类型知识库，涵盖都市异能、重生、系统流、商战、职场、现实逻辑约束

**Architecture:** 在 `templates/knowledge-base/genres/` 目录下新建 `urban.md`。

**Tech Stack:** Markdown 知识库

---

### Task 23.1: 创建 urban.md

**Files:**
- Create: `templates/knowledge-base/genres/urban.md`

**Step 1: 创建文件**

```markdown
# 都市小说 (Urban Fiction)

## 快速参考

### 核心定义

**都市小说**：以现代都市为背景的小说类型。核心特征是「现实世界 + 超常元素（或纯现实冲突）」。读者代入感强，因为背景是熟悉的现代社会。

**与玄幻的核心区别**：
- 玄幻：架空世界，规则由作者定义
- 都市：现实世界，必须遵守基本社会规则（即使有超能力）

### 子类型速查

| 子类型 | 核心卖点 | 典型设定 |
|--------|---------|---------|
| 都市异能 | 现实世界中的超能力 | 觉醒异能、隐藏的超自然世界 |
| 都市重生 | 先知优势 + 人生重来 | 重生回到过去，利用记忆逆袭 |
| 都市系统 | 系统辅助 + 任务奖励 | 获得系统，完成任务变强 |
| 商战/职场 | 智谋博弈 + 权力斗争 | 商业竞争、职场晋升 |
| 都市修仙 | 修仙体系 + 现代社会 | 末法时代、灵气复苏 |
| 纯都市 | 现实冲突 + 人生百态 | 创业、家庭、社会问题 |

---

## 核心要素

### 1. 现实逻辑约束

**最重要的原则**：都市小说必须遵守基本的现实逻辑。

**必须遵守的规则**：
- 法律存在且有效（不能随意杀人而无后果）
- 社会秩序基本正常（不是末世）
- 经济规律有效（不能凭空变出钱）
- 科技水平与现实一致
- 人际关系符合现代社会规范

**可以突破的规则**（如有超自然设定）：
- 物理法则（异能/修仙）
- 信息获取方式（系统/重生记忆）
- 个人能力上限（超能力）

**常见错误**：
- ❌ 主角在闹市区大打出手，无人报警
- ❌ 主角杀人后毫无法律后果
- ❌ 主角一夜暴富，无人追查资金来源
- ❌ 主角在学校/公司展示超能力，无人在意

### 2. 身份管理

**核心冲突**：主角拥有超常能力/知识，但必须在正常社会中生活。

**身份管理维度**：
| 维度 | 问题 | 张力来源 |
|------|------|---------|
| 能力隐藏 | 如何在人前隐藏超能力？ | 差点暴露的紧张感 |
| 财富来源 | 如何解释突然的财富？ | 被调查的风险 |
| 社会关系 | 如何维持正常的人际关系？ | 秘密带来的隔阂 |
| 双重生活 | 如何平衡普通生活和超常活动？ | 时间冲突、精力分配 |

### 3. 都市爽点设计

**都市爽点与玄幻爽点的区别**：
- 玄幻爽点：碾压式战力差距
- 都市爽点：社会地位反转、智谋碾压、身份揭示

**都市特有爽点类型**：
| 类型 | 描述 | 示例 |
|------|------|------|
| 身份反转 | 被看不起的人其实是大佬 | 扮猪吃虎、隐藏身份 |
| 商业碾压 | 用商业手段击败对手 | 收购对方公司、断供应链 |
| 社会地位逆袭 | 从底层到顶层 | 穷小子变富豪、小职员变 CEO |
| 知识碾压 | 用先知/专业知识碾压 | 重生者预知股市、专家打脸外行 |
| 关系反转 | 曾经的关系发生逆转 | 前女友后悔、前老板求合作 |

### 4. 都市节奏特点

**与玄幻的节奏差异**：
- 玄幻：修炼 → 突破 → 打脸（循环）
- 都市：事件 → 应对 → 结果 → 新事件（更接近现实节奏）

**都市节奏建议**：
- 每章至少一个「事件」（不能只有日常描写）
- 商战/职场类：每 3-5 章一个小胜利
- 异能类：每 5-8 章一次能力展示/战斗
- 重生类：每 2-3 章一次「先知优势」的运用

---

## 子类型详解

### 都市重生

**核心机制**：主角带着前世记忆重生到过去

**关键设计点**：
1. **先知优势管理**：
   - 不能让主角什么都记得（记忆应该模糊/有限）
   - 蝴蝶效应：主角的行动会改变未来，先知优势逐渐失效
   - 先知优势的使用应该有代价（引起注意、改变命运线）

2. **重生时间点选择**：
   - 太早：需要大量铺垫才能到达关键事件
   - 太晚：错过太多改变机会
   - 建议：重生到关键转折点前 1-3 年

3. **情感处理**：
   - 前世的感情如何处理？（前世爱人、前世仇人）
   - 心理年龄与生理年龄的矛盾
   - 「第二次机会」的珍惜感

### 都市系统

**核心机制**：主角获得某种系统，通过完成任务获得奖励

**关键设计点**：
1. **系统限制**：不能告诉别人、任务失败有惩罚、系统有冷却
2. **任务设计**：任务应该推动剧情，而非与剧情脱节
3. **奖励平衡**：奖励不能太强（破坏现实感），也不能太弱（失去吸引力）

### 商战/职场

**核心机制**：通过商业智谋和职场博弈实现目标

**关键设计点**：
1. **专业性**：商业逻辑必须基本合理（读者中有商业人士）
2. **对手智商**：对手不能太蠢，否则胜利没有含金量
3. **团队建设**：主角不能一个人做所有事，需要团队
4. **风险与代价**：商业决策必须有风险，不能每次都赢

---

## 常见陷阱

### 1. 现实感崩塌
❌ 主角行为完全脱离现实社会规则
- 解决：每个超常行为都要考虑社会后果

### 2. 金手指过强
❌ 系统/重生记忆让主角毫无挑战
- 解决：设计金手指失效的场景

### 3. 配角智商下线
❌ 所有人都被主角轻松碾压
- 解决：给对手合理的优势和智谋

### 4. 感情线生硬
❌ 女主角毫无理由地爱上主角
- 解决：感情发展需要合理的互动和契机

### 5. 节奏拖沓
❌ 大量日常描写没有推进剧情
- 解决：每个日常场景都要有「信息释放」或「关系推进」

---

## 检测清单

### 开篇检查
- [ ] 现代都市背景是否建立？
- [ ] 主角的日常生活是否有代入感？
- [ ] 超常元素（如有）是否在前 3 章引入？
- [ ] 核心冲突是否与都市背景相关？

### 每卷检查
- [ ] 现实逻辑是否保持一致？
- [ ] 身份管理是否有张力？
- [ ] 都市爽点是否充足？
- [ ] 配角是否有合理的智商和动机？
```

**Step 2: Commit**

```bash
git add templates/knowledge-base/genres/urban.md
git commit -m "feat(kb): add urban.md genre knowledge base (urban fantasy, rebirth, system, business, reality constraints)"
```
