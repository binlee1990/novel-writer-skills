# Task 5: 新增 `xuanhuan.md` 知识库

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** 创建玄幻小说类型知识库，涵盖力量体系设计、境界划分、宗门体系、天材地宝、大能设定等核心内容

**Architecture:** 在 `templates/knowledge-base/genres/` 目录下新建 `xuanhuan.md`，遵循现有类型知识库的结构风格（参考 `wuxia.md`、`romance.md` 等）。文件包含快速参考、核心要素、力量体系设计、宗门/势力体系、常见套路与创新、常见陷阱、检测清单。

**Tech Stack:** Markdown 知识库

---

### Task 5.1: 创建 xuanhuan.md

**Files:**
- Create: `templates/knowledge-base/genres/xuanhuan.md`

**Step 1: 创建文件**

```markdown
# 玄幻小说 (Xuanhuan / Fantasy)

## 快速参考

### 核心定义

**玄幻小说**：以东方修炼体系为基础，融合奇幻元素的小说类型。核心驱动力是「变强」——主角通过修炼、机缘、战斗不断突破，最终站在世界之巅。

**与武侠的区别**：
- 武侠：人体极限内的武功，江湖恩怨，侠义精神
- 玄幻：超越人体极限的修炼，天地法则，逆天改命

**核心吸引力**：
- 变强的快感（升级）
- 碾压的爽感（打脸）
- 探索的新奇感（新地图、新境界）
- 逆袭的代入感（废物变天才）

---

## 核心要素

### 1. 力量体系（最重要）

**设计原则**：
- **层级清晰**：每个境界的能力边界明确，读者能直观理解强弱差距
- **突破有感**：每次突破都有仪式感和实质性的能力提升
- **上限可见**：读者知道最强是什么级别，但不知道主角能不能到达
- **越级有代价**：越级战斗必须有合理解释和代价

**经典境界模板**（9 级制）：

| 大境界 | 子境界 | 能力特征 | 社会地位 |
|--------|--------|---------|---------|
| 凡人境 | 炼体 → 聚气 → 开脉 | 超越常人体质，感应灵气 | 普通修士 |
| 灵修境 | 筑基 → 金丹 → 元婴 | 飞行、法术、领域 | 宗门核心 |
| 天人境 | 化神 → 渡劫 → 大乘 | 移山填海、空间法则 | 一方霸主 |

**注意**：以上仅为模板，应根据故事需要自定义。关键是保持内部一致性。

**境界设计检查清单**：
- [ ] 每个境界的能力上限是否明确？
- [ ] 相邻境界的差距是否合理？（不能太小也不能太大）
- [ ] 突破条件是否清晰？（资源 + 悟性 + 机缘）
- [ ] 是否有「瓶颈」设计？（不是所有人都能突破）
- [ ] 越级战斗的条件是否设定？（特殊体质、神器、秘术）

### 2. 战力标准

**战力描写原则**：
- **具象化**：不说「很强」，说「一拳打碎山壁」
- **对比化**：通过已知参照物展示强弱（「他一个人，顶得上十个金丹期」）
- **一致性**：同境界的战力描写不能忽高忽低

**战力崩坏的常见原因**：
- 前期小境界差距巨大，后期大境界差距反而缩小
- 配角战力随剧情需要忽强忽弱
- 主角越级战斗太频繁，导致境界失去意义
- 新出场的强者战力与已有体系矛盾

**避免战力崩坏的方法**：
- 维护一份「战力参照表」（在 `world-setting.json` 中）
- 每次战斗前检查双方境界和特殊条件
- 越级战斗必须有明确的「外挂」解释（神器、血脉、秘术）
- 定期回顾早期战力描写，确保一致

### 3. 升级节奏

**升级节奏公式**：

```
前期（1-100章）：升级快，每 5-10 章突破一次小境界
中期（100-300章）：升级放缓，每 15-30 章突破一次
后期（300+章）：升级极慢，每 50-100 章突破一次大境界
```

**升级节奏的核心矛盾**：
- 太快：读者没有期待感，境界变得廉价
- 太慢：读者等不及，感觉拖沓
- 解决方案：用「小突破」（技能、法宝、领悟）填充大境界之间的空白

**升级事件类型**：
| 类型 | 频率 | 爽感 | 示例 |
|------|------|------|------|
| 境界突破 | 低 | 极高 | 筑基 → 金丹 |
| 技能领悟 | 中 | 高 | 领悟新剑意 |
| 法宝获得 | 中 | 高 | 得到上古神器 |
| 体质觉醒 | 极低 | 极高 | 血脉觉醒 |
| 战斗领悟 | 高 | 中 | 战斗中突破瓶颈 |

### 4. 金手指设计

**金手指**：主角区别于其他角色的核心优势（系统、传承、特殊体质等）。

**金手指设计原则**：
- **有限制**：不能无限使用，必须有冷却/代价/条件
- **可成长**：金手指本身也需要升级，与主角同步成长
- **有代价**：使用金手指必须付出某种代价（体力、寿命、秘密暴露风险）
- **不万能**：金手指解决不了所有问题，主角仍需自身努力

**常见金手指类型**：
| 类型 | 优势 | 限制示例 |
|------|------|---------|
| 系统 | 任务奖励、属性面板 | 任务失败惩罚、不能告诉别人 |
| 传承 | 前辈记忆、功法 | 需要逐步解锁、有副作用 |
| 特殊体质 | 天赋异禀 | 初期是诅咒、需要特殊资源激活 |
| 空间 | 储物、种植、时间加速 | 容量有限、进入条件苛刻 |
| 重生 | 先知优势 | 蝴蝶效应、记忆模糊、时间线偏移 |

---

## 宗门/势力体系

### 势力层级模板

```
个人 → 家族 → 宗门 → 王朝/帝国 → 圣地/禁区 → 上界/更高位面
```

### 宗门结构模板

| 层级 | 职位 | 境界要求 | 权力 |
|------|------|---------|------|
| 最高层 | 宗主/太上长老 | 最高境界 | 决策权 |
| 管理层 | 长老/执事 | 高境界 | 管理权 |
| 核心层 | 内门弟子/真传 | 中境界 | 资源优先 |
| 基础层 | 外门弟子 | 低境界 | 基础资源 |
| 底层 | 杂役/记名 | 无要求 | 极少资源 |

### 势力关系设计

**核心原则**：势力之间的关系 = 利益 + 历史恩怨 + 实力对比

**常见关系模式**：
- 三足鼎立（稳定但脆弱）
- 一超多强（主角需要打破格局）
- 联盟对抗（阵营对立）
- 暗流涌动（表面和平，暗中博弈）

---

## 地图与空间设计

### 地图扩展原则

**核心**：地图随主角实力扩展，每个新区域对应新的境界层次。

```
新手村（凡人境）→ 小城/宗门（灵修境初期）→ 王朝/大陆（灵修境后期）
→ 上界/星域（天人境）→ 更高位面（终极境界）
```

**每个区域必须有**：
- 独特的环境特征（沙漠、冰原、火山）
- 对应的资源和机缘
- 独特的势力格局
- 主角在此区域的目标

---

## 常见套路与创新

### 经典套路

| 套路 | 描述 | 爽点 |
|------|------|------|
| 废物逆袭 | 被看不起 → 觉醒 → 打脸 | 逆袭快感 |
| 退婚流 | 被退婚 → 变强 → 对方后悔 | 打脸 + 逆袭 |
| 宗门大比 | 比武 → 越级挑战 → 震惊全场 | 碾压 + 震惊 |
| 秘境探险 | 进入秘境 → 获得机缘 → 突破 | 探索 + 升级 |
| 拍卖会 | 竞价 → 冲突 → 展示实力/财力 | 装逼 + 打脸 |

### 创新方向

- **反套路**：主角不是废物，而是从强者开始（但有隐藏弱点）
- **群像玄幻**：不只是主角变强，配角也有完整的成长线
- **硬核修炼**：修炼体系有严密的逻辑和科学感
- **日常向**：不以战斗为主，而是修炼世界的日常生活
- **反派视角**：从反派角度讲述故事

---

## 常见陷阱

### 1. 境界通胀

❌ 前期一个金丹期就是大人物，后期金丹期满地走
- 解决：提前规划好每个境界在世界中的稀有度

### 2. 战力崩坏

❌ 主角前期苦战筑基期，后期轻松碾压同境界
- 解决：维护战力参照表，每次战斗前核对

### 3. 配角工具化

❌ 配角只在需要被打脸时出现
- 解决：给重要配角独立的目标和成长线

### 4. 地图废弃

❌ 主角离开某个区域后，该区域再也不提
- 解决：通过消息、回忆、旧友来访等方式保持联系

### 5. 金手指万能

❌ 所有问题都靠金手指解决
- 解决：设计金手指解决不了的问题类型（人际关系、道德困境）

### 6. 升级单调

❌ 每次升级都是「闭关 → 突破 → 出关」
- 解决：设计多种升级方式（战斗领悟、奇遇、顿悟、传承）

---

## 检测清单

### 开篇检查
- [ ] 力量体系是否在前 3 章内让读者理解基本框架？
- [ ] 主角的起点和金手指是否在前 5 章内明确？
- [ ] 第一个小目标是否清晰？（打败某人、通过考核、获得资格）

### 每卷检查
- [ ] 本卷是否有至少 1 次境界突破？
- [ ] 本卷是否引入了新的地图/势力？
- [ ] 本卷的核心冲突是否与力量体系相关？
- [ ] 战力描写是否与之前保持一致？

### 全书检查
- [ ] 境界体系是否从头到尾一致？
- [ ] 金手指的限制是否始终被遵守？
- [ ] 是否有被遗忘的配角或势力？
- [ ] 升级节奏是否符合预期曲线？
```

**Step 2: 验证文件创建**

Run: `ls -la templates/knowledge-base/genres/xuanhuan.md`
Expected: 文件存在

Run: `wc -l templates/knowledge-base/genres/xuanhuan.md`
Expected: 约 250-300 行

**Step 3: Commit**

```bash
git add templates/knowledge-base/genres/xuanhuan.md
git commit -m "feat(kb): add xuanhuan.md genre knowledge base (power systems, sects, web novel patterns)"
```
