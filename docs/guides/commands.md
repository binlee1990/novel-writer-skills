...
自动重建创作上下文，汇总角色状态、未解决伏笔、情节进展和关系网络，生成结构化简报。
...
```
/recap          # 完整 8 部分简报
/recap --brief  # 仅输出快速参考卡片
```

### 数据来源

读取 `spec/tracking/` 下的追踪数据和最近章节内容：
- `character-state.json` - 角色状态
- `plot-tracker.json` - 情节和伏笔
...
- **伏笔优先级排序**：超过 15 章未处理的标记为高优先级
- **角色缺席警告**：重要角色超过 10 章未出场时警告
- **节奏评估**：分析最近 5 章的事件密度
- **关系趋势分析**：展示最近的关系变化方向
